...
RSpec.describe "Habitation details", type: :request do
  ...
  describe "GET /imoveis/:id" do
    ...
    it "does not expose broker phone or direct whatsapp link in the responsible attendant card" do
      ...
      expect(response.body).to include('data-action="click->lead-capture#open"')
      expect(response.body).to include('data-force-lead-capture="true"')
      expect(response.body).to include("data-share-token=\"#{share_link.token}\"")
