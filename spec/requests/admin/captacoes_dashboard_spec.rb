...
RSpec.describe "Admin::Captacoes dashboard", type: :request do
  ...
  let(:period_start) { Date.new(2035, 1, 1) }
  let(:period_end) { Date.new(2035, 1, 31) }
  ...
  it "calcula metas e indicadores de captação a partir dos imóveis" do
    create_dashboard_habitation(valor_venda_cents: 900_000_00, valor_locacao_cents: 0, regiao_foco: "Centro")
    create_dashboard_habitation(valor_venda_cents: 700_000_00, valor_locacao_cents: 0, regiao_foco: "Não")
    create_dashboard_habitation(valor_venda_cents: 0, valor_locacao_cents: 4_000_00, regiao_foco: "Sim", salute_rental_management_flag: true)
    create_dashboard_habitation(valor_venda_cents: 0, valor_locacao_cents: 3_000_00, regiao_foco: "Não", salute_rental_management_flag: false)

    get dashboard_admin_captacoes_path(start_date: period_start, end_date: period_end)

    expect(response).to have_http_status(:ok)
    expect(response.body).to include("Região foco Venda")
    expect(response.body).to include("Região foco Locação")
    expect(response.body).to include("Rascunho/Devolvido")
  ...
  def create_dashboard_habitation(attributes = {})
    create(
      :habitation,
      {
        admin_user: admin,
        codigo: "CAPT-DASH-#{SecureRandom.hex(6)}",
        data_cadastro_crm: period_start.noon
      }.merge(attributes)
    )
  end
end
